  ...
  <div class="app-container">
    <Suspense>
      <AppSidebar :curr-convo="route.params.id" :messages="[]" :is-open="sidebarOpen"
        ...
        @reload-settings="settingsManager.loadSettings" @open-settings="openSettingsPanel('models')" />
      ...
      <DialogPortal>
        ...
        <DialogContent class="dialog-content">
          <SettingsPanel :is-open="isSettingsOpen" :initial-tab="settingsInitialTab"
            @close="isSettingsOpen = false; settingsInitialTab = 'models';"
...
const settingsInitialTab = ref('models'); // Controls which tab opens in settings panel

// Set up dynamic page title
const title = computed(() => {
  if (route.params.id) {
    return `${route.params.id} - LinChat`;
  } else if (route.path === '/' || route.path === '/new') {
    return 'New Chat - LinChat';
  } else {
    return 'LinChat';
  }
});
...
function openSettingsPanel(tabKey = 'models') {
  console.log('openSettingsPanel called with tab:', tabKey);
  settingsInitialTab.value = tabKey;
  isSettingsOpen.value = true;
  console.log('isSettingsOpen set to:', isSettingsOpen.value);
}
