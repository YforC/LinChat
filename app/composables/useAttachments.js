 ...
 * @param {string} storageKey - Key used to persist attachments in localforage
 * @returns {Object} Attachment state and methods
 */
export function useAttachments(storageKey = 'pending_attachments') {
    ...
    // Save to storage whenever attachments change
    const saveAttachments = async () => {
        if (typeof window !== 'undefined') {
            await localforage.setItem(storageKey, JSON.parse(JSON.stringify(attachments.value)));
        }
    };
    ...
    async function addFile(file, supportsVision = false) {
        ...
        try {
            ...
            if (processedSize > MAX_FILE_SIZE) {
                ...
                error.value = `File "${file.name}" is ${sizeMB}MB after processing, which exceeds the ${MAX_FILE_SIZE_MB}MB limit.`;
                return { success: false, error: error.value };
            }
