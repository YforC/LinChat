...
/**
 * Manages application settings for the LinChat Interface.
 */
class Settings {
  constructor() {
  ...
  async loadSettings() {
    try {
      ...
      if (savedSettings != null) {
        ...
        const resolvedModel = findModelById(availableModels, mergedSettings.selected_model_id);
        if (!resolvedModel) {
          let fallbackModelId = DEFAULT_MODEL_ID;
          const modelsList = availableModels.value || [];
          for (const item of modelsList) {
            if (item?.models?.length) {
              fallbackModelId = item.models[0].id;
              break;
            }
            if (item?.id) {
              fallbackModelId = item.id;
              break;
            }
          }
          mergedSettings.selected_model_id = fallbackModelId;
        }
  ...
  get selectedModelName() {
    console.log('Getting selectedModelName');
